  ...
  try {
    const blog = await Blog.findAll({
      ...
      include: [
        ...
        {
          model: Staff,
          attributes: ["staff_id", "Name", "Phone", "Avatar", "Email"],
        },
      ],
    });
    return blog;
  } catch (error) {
    return res.status(500).json({ message: "getall failed: " + error.message });
  }
}
...
async function createBlog(data, res) {
  try {
    ...
    console.error(err);
    return { success: false, message: "Failed to create blog: " + err.message };
  }
}
